...
// Workspace logo upload. Same stack0 CDN flow as uploadEmailImage, gated on the
// org rather than an event: workspace branding is not event-scoped, and the
// public /<org-slug> page fetches the logo anonymously, so app-hosted
// owner-scoped storage cannot serve it.
const MAX_IMAGE_BYTES = 4 * 1024 * 1024;
const STACK0_API = "https://api.stack0.dev";

export default action<
  { orgId: string; filename: string; mimeType: string; dataBase64: string },
  { url: string }
>({
  timeout: 60,
  args: {
