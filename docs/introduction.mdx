...
<Note>
  This reference is generated from the running application's manifest with
  `pylon codegen openapi`, not written by hand. If an endpoint is listed here,
  the router serves it; functions marked internal are excluded because the
  router refuses external calls to them.
</Note>
