...
test("valid blob parses", () => {
  ...
  expect(b).toEqual({ accent: "#7c3aed", logoUrl: "https://x.com/l.svg", tagline: "Two days of AI" });
});

test("malformed JSON degrades to defaults", () => {
  expect(parseBranding("{oops")).toEqual({ accent: null, logoUrl: null, tagline: null });
  expect(parseBranding(undefined)).toEqual({ accent: null, logoUrl: null, tagline: null });
  expect(parseBranding("")).toEqual({ accent: null, logoUrl: null, tagline: null });
});

test("bad accent and unsafe logo URLs are dropped", () => {
  const b = parseBranding({ accent: "red", logoUrl: "javascript:alert(1)", tagline: "" });
  expect(b).toEqual({ accent: null, logoUrl: null, tagline: null });
